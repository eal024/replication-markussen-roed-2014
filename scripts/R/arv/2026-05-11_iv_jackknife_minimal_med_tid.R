...
#
# Utvidelse av 2026-05-08_iv_jackknife_minimal.R: legger til tidsdimensjon
# (person-måned, ligning 2 → 3 → 4/5). Egen versjon skrevet av Eirik som
# læringsøvelse — beholder M&R-notasjon. Fasit-skript finnes i
# 2026-05-08_iv_jackknife_med_tid.R (skal ikke åpnes før etter forsøk).
#
...
# 2b. Kalibrering av base-hazarden ----------------------------------------
# Geometrisk overgangsmodell:
#   t_event ~ rgeom(h) + 1
#   P(sensurert ved T mnd) = (1 - h)^T
#   P(behandlet innen T mnd) = 1 - (1 - h)^T
#
# Inverteringsformelen — gitt ønsket andel behandlet, regn ut h:
#   h = 1 - (1 - andel)^(1/T)
#
# Referansetabell (T = 24 mnd):
#
#   andel behandlet | h
#   ----------------|--------
#       25 %        | 0.012
#       30 %        | 0.015   ← M&R: ~27 % over 4 år, vi sikter mot ~30 % i 24 mnd
#       50 %        | 0.029
#       75 %        | 0.057
#       90 %        | 0.092
#
# Valg: h_base = 0.015 (matcher fasit-skriptet og er nærmest artikkelen).

max_months <- 24
h_base     <- 0.015

# Sanity — sjekk forventet andel behandlet for valgt h_base:
1 - (1 - h_base)^max_months
